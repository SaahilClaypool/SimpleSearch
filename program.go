...
func main() {
	...
	searches := &Searches{}
	err := webSearchAgent.Json(fmt.Sprintf("What google search queries would you perform to answer this query? Provide up to 3 queries. Make the queries varied. Target different domain specific keywords or sites (for example, 'stackoverflow.com how do I invert a linked list?'.\n----\nOriginal Request: %s", input), searches)
	if err != nil {
		panic(err)
	}
